...
namespace teacherKeyboards
{
    InlineKeyboardMarkup::Ptr create_comments_kb(long chat_id)
    {
        ...
        std::string query = std::format("SELECT l.id, TO_CHAR(l.date, 'dd/mm/yyyy'), \
        ...
        for (auto it{R.begin()}; it!=R.end(); ++it)
        {
            ...
            comment_btn->text = std::format("{} {} {}", date, first_name, last_name);
            comment_btn->callbackData = std::format("comment_for_teacher {}", id);
    ...
    InlineKeyboardMarkup::Ptr create_teacher_start_kb(bool is_admin)
    {
        ...
        if (is_admin)
        {
            InlineKeyboardButton::Ptr list_teacher_btn(new InlineKeyboardButton);
    ...
    InlineKeyboardMarkup::Ptr create_pupils_kb(long chat_id)
    {
        ...
        InlineKeyboardButton::Ptr teacher_active_pupils_btn(new InlineKeyboardButton);
    ...
    InlineKeyboardMarkup::Ptr create_teachers_kb()
    {
        ...
        InlineKeyboardButton::Ptr teacher_active_teachers_btn(new InlineKeyboardButton);
    ...
    InlineKeyboardMarkup::Ptr create_list_users_kb(long chat_id, bool is_active)
    {
        ...
        {
            query = std::format(
                "SELECT * FROM bot_user WHERE teacher={} AND is_active={} ORDER BY last_name", 
                chat_id, 
                is_active);
        }
        else
        ...
        for (auto it{user_list.begin()}; it!=user_list.end(); ++it)
        {
            ...
            user_btn->text = std::format("{} {}", it->first_name, it->last_name);
    ...
    InlineKeyboardMarkup::Ptr create_user_info_kb(botUser& u)
    {
        ...
        if (!u.is_active)
        {
            ...
            activate_btn->callbackData = std::format("activate_user {}", u.chat_id);
        ...
        else
        {
            ...
            update_btn->callbackData = std::format("update_user {} {}", u.role, u.chat_id);
    ...
    InlineKeyboardMarkup::Ptr update_user_info_kb(std::string role, long chat_id)
    {
        InlineKeyboardMarkup::Ptr keyboard(new InlineKeyboardMarkup);
        vector<InlineKeyboardButton::Ptr> row;
        
        InlineKeyboardButton::Ptr update_first_name_btn(new InlineKeyboardButton);
        update_first_name_btn->text = "Изменить имя";
        update_first_name_btn->callbackData = std::format("update_user_field first_name {}", chat_id);
        row.push_back(update_first_name_btn);
        keyboard->inlineKeyboard.push_back(row);
        row.clear();
        
        InlineKeyboardButton::Ptr update_last_name_btn(new InlineKeyboardButton);
        update_last_name_btn->text = "Изменить фамилию";
        update_last_name_btn->callbackData = std::format("update_user_field last_name {}", chat_id);
        ...
        if (role == "pupil")
        {
            InlineKeyboardButton::Ptr update_class_btn(new InlineKeyboardButton);
            update_class_btn->text = "Изменить класс";
            update_class_btn->callbackData = std::format("update_user_field class {}", chat_id);
        ...
        update_phone_btn->callbackData = std::format("update_user_field phone {}", chat_id);
        ...
        update_email_btn->callbackData = std::format("update_user_field email {}", chat_id);
        ...
        update_comments_btn->callbackData = std::format("update_user_field comments {}", chat_id);
        ...
        finish_update_btn->callbackData = std::format("update_user_field finish {}", chat_id);
    ...
    InlineKeyboardMarkup::Ptr create_list_pupils_kb(long chat_id, bool is_active, bool update)
    {
        ...
        for (auto it{user_list.begin()}; it!=user_list.end(); ++it)
        {
            ...
            InlineKeyboardButton::Ptr user_btn(new InlineKeyboardButton);
            user_btn->text = std::format("{} {}", it->first_name, it->last_name);
            user_btn->callbackData = std::vformat(callback_data, std::make_format_args(it->chat_id));
            row.push_back(user_btn);
            keyboard->inlineKeyboard.push_back(row);
        }
    ...
    InlineKeyboardMarkup::Ptr update_lesson_info_kb(int lesson_id)
    {
        ...
        InlineKeyboardButton::Ptr update_lesson_date_btn(new InlineKeyboardButton);
        update_lesson_date_btn->text = "Изменить дату";
        update_lesson_date_btn->callbackData = std::format("update_lesson_field date {}", lesson_id);
        row.push_back(update_lesson_date_btn);
        keyboard->inlineKeyboard.push_back(row);
        row.clear();
        
        InlineKeyboardButton::Ptr update_lesson_pupil_btn(new InlineKeyboardButton);
        update_lesson_pupil_btn->text = "Изменить ученика";
        update_lesson_pupil_btn->callbackData = std::format("update_lesson_field pupil {}", lesson_id);
        row.push_back(update_lesson_pupil_btn);
        keyboard->inlineKeyboard.push_back(row);
        row.clear();
        
      
        InlineKeyboardButton::Ptr update_lesson_objectives_btn(new InlineKeyboardButton);
        update_lesson_objectives_btn->text = "Изменить тему занятия";
        update_lesson_objectives_btn->callbackData = std::format("update_lesson_field objectives {}", lesson_id);
        row.push_back(update_lesson_objectives_btn);
        keyboard->inlineKeyboard.push_back(row);
        row.clear();
     
        
        InlineKeyboardButton::Ptr update_lesson_pupil_comments_btn(new InlineKeyboardButton);
        update_lesson_pupil_comments_btn->text = "Изменить комментарий для ученика";
        update_lesson_pupil_comments_btn->callbackData = std::format("update_lesson_field comment_for_pupil {}", lesson_id);
        row.push_back(update_lesson_pupil_comments_btn);
        keyboard->inlineKeyboard.push_back(row);
        row.clear();

        InlineKeyboardButton::Ptr update_lesson_teacher_comments_btn(new InlineKeyboardButton);
        update_lesson_teacher_comments_btn->text = "Изменить комментарий для учителя";
        update_lesson_teacher_comments_btn->callbackData = std::format("update_lesson_field comment_for_teacher {}", lesson_id);
        ...
        InlineKeyboardButton::Ptr finish_update_btn(new InlineKeyboardButton);
        finish_update_btn->text = "Закончить редактирование";
        finish_update_btn->callbackData = std::format("update_lesson_field finish_lesson_update {}", lesson_id);
        row.push_back(finish_update_btn);
        keyboard->inlineKeyboard.push_back(row);
        row.clear();
